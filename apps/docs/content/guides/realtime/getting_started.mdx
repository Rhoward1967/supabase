...
Supabase Realtime enables you to build collaborative, multiplayer, and real-time applications with ease. Whether you're creating a chat application, collaborative editor, live dashboard, or multiplayer game, Realtime provides the tools you need.
...
- **Presence** - Track and synchronize user state across clients. Ideal for showing who's online, user cursors, or active participants.
...
First, get your project URL and key from your [Supabase Dashboard](/dashboard/project/_/settings/api).
...
const supabase = createClient('https://<project>.supabase.co', '<your-key>')
```
...
final channel = supabase.channel(
  'room:lobby:messages',
  opts: RealtimeChannelConfig(private: true), // Recommended for production
);
```
...
```swift
// Create a channel with a descriptive topic name
let channel = supabase.realtimeV2.channel("room:lobby:messages")
```
...
```python
# Create a channel with a descriptive topic name
channel = supabase.channel('room:lobby:messages')
```
